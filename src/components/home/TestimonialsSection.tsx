...
export default function TestimonialsSection() {
  ...
  return (
    <section className="relative py-24 overflow-hidden section-mixed">
      ...
      <div className="absolute top-1/2 left-0 w-72 h-72 rounded-full bg-gold/[0.02] blur-[150px]" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {t.testimonials.items.map((item, i) => (
            <StaggerItem key={i}>
              <motion.div
                className="glass rounded-2xl p-8 h-full card-hover relative border border-white/[0.04] hover:border-gold/15 transition-all"
              ...
              >
                ...
                <div className="absolute top-8 right-8 flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, j) => (
                    <svg key={j} className="w-3.5 h-3.5 text-gold/60" fill="currentColor" viewBox="0 0 20 20">
